...
export class OSS {
  ...
  async upload(patterns: string | string[], statics: string): Promise<boolean> {
    ...
    const tasks = files.map((file: string) => {
      ...
      return new Promise(async (resolve, reject) => resolve(await this.client.put(name, file)))
        .then((res: any)  => Object.assign({}, res, res && res.res || {}));
    });

    log(chalk.yellow(`Upload begin, size of task pool: ${ TaskPoolConfig.size }\n`));

    return new Promise((resolve, reject) => {
      const taskPool = new TaskPool(tasks, {
        size: TaskPoolConfig.size,
        onEnd: (status: TaskPoolStatusInfo) => {
          log(chalk.yellow(`Upload finished, total: ${ chalk.white(status.total + '') }, succeed: ${ chalk.green(status.succeed + '') }, failed: ${ chalk.gray(status.failed + '') }\n`));
          resolve(status.failed === 0);
        },
        onEachEnd: (res: any, status: TaskPoolStatusInfo) => log(chalk.cyan(`    upload progress: ${
          chalk.white(Math.floor(status.succeed * 100 / status.total) + '%')
        } (${ chalk.green(status.succeed + '') }/${ chalk.white(status.total + '') }) ${ res.status === 200 ? chalk.yellow('succeed: ') : chalk.red('failed: ') }${ chalk.underline(res.name) }`))
      });
  ...
  async delete(patterns: string | string[], statics: string): Promise<boolean> {
    ...
    log(chalk.yellow(`Multi delete finished ${ succeed ? chalk.green('succeed! ') : chalk.red('failed!') }\n`));
  ...
  async remove() {
    ...
    log(`Remove all files in bucket ${ this.config.bucket } ${ succeed ? chalk.green('succeed') : chalk.red('failed') }.\n`);
  ...
  async list(): Promise<string[]> {
    ...
    return result.res && result.res.status === 200 ? (result.objects && result.objects || []).map((item: any) => item.name) : [];
  }
